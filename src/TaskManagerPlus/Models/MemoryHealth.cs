...
/// "never run") when no such event exists in the retained log window, not "passed". Named
/// "Latest" (rather than plain MemoryDiagnosticResultInfo) since item 99's
/// MemoryDiagnosticResultInfo in RecoveryModels.cs already owns that name for the Windows Health
/// tab's full run-history list - both parse the same event source independently for two different
/// cards (single most-recent result here vs. every retained run there).</summary>
public sealed class LatestMemoryDiagnosticResult
{
    public DateTime TimeCreated { get; init; }
