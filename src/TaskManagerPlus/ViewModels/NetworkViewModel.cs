...
public sealed class NetworkTroubleshootReport
{
    ...
    public List<NetworkFirewallRuleInfo> MatchingFirewallRules { get; init; } = new();
...
public sealed class NetworkViewModel : ObservableObject, IDisposable
{
    ...
    // #566: profile status.
    public ObservableCollection<NetworkFirewallProfileStatus> FirewallProfiles { get; } = new();
    ...
    private List<NetworkFirewallRuleInfo> _allFirewallRules = new();
    public ObservableCollection<NetworkFirewallRuleInfo> FilteredFirewallRules { get; } = new();
    ...
    private async Task RefreshFirewallProfilesAsync()
    {
        ...
        try
        {
            var profiles = await NetworkFirewallService.ReadProfileStatusAsync();
    ...
    private async Task RefreshFirewallRulesAsync()
    {
        ...
        try
        {
            _allFirewallRules = await NetworkFirewallService.ReadRulesAsync();
    ...
    {
        var filtered = NetworkFirewallService.FilterRules(_allFirewallRules, FirewallRuleFilterText, FirewallFilterExecutableOnly);
    ...
    private async Task RunWizardAsync()
    {
        ...
        try
        {
            var rules = await NetworkFirewallService.ReadRulesAsync();
            var matchingRules = NetworkFirewallService.FilterRules(rules, query, executableOnly: true);
