...
public sealed class DevicesDriversViewModel : ObservableObject
{
    ...
    private DriverVerifierConfigStatus? _verifierStatus;
    public DriverVerifierConfigStatus? VerifierStatus { get => _verifierStatus; private set => SetProperty(ref _verifierStatus, value); }
    ...
    private async Task LoadVerifierStatusAsync()
    {
        ...
        try
        {
            VerifierStatus = await DriverVerifierControlService.QueryStatusAsync();
            ...
            var (safeBootConfigured, safeBootMode) = await DriverVerifierControlService.QuerySafeBootAsync();
    ...
    private async Task EnableVerifierAsync(List<string> fileNames)
    {
        ...
        try
        {
            var (success, message) = await DriverVerifierControlService.EnableStandardAsync(fileNames);
    ...
    private async Task ResetVerifierAsync()
    {
        ...
        try
        {
            var (success, message) = await DriverVerifierControlService.ResetAsync();
            VerifierActionStatusText = success
    ...
    private async Task ToggleSafeBootAsync(bool enable)
    {
        ...
        try
        {
            var (success, message) = await DriverVerifierControlService.SetSafeBootAsync(enable);
            VerifierActionStatusText = success
                ? (enable
