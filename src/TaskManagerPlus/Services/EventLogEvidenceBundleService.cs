...
/// Named "Security" here (rather than plain <see cref="EventLogEvidenceBundleService"/> below)
/// because it's a narrower, security-tab-scoped bundle of already-loaded session state, distinct
/// from that class's full evtx/WER/dump/systeminfo export.
/// </summary>
public static class SecurityEvidenceBundleService
{
...
/// <summary>
/// #200: "filtered evidence bundle export" - the integration point for most of the Events tab's
/// other services: EventTimelineService (the unified timeline), WerReportService (crash report
/// copies), EventLogService (the minidump list), EventKnowledgeBaseService (KB explanations for the
/// top signatures), and EventLogExplorerService (the actual .evtx exports). Produces a timestamped
...
/// Named "EventLog" here (rather than plain EvidenceBundleService) since item #981-986's
/// EvidenceBundleService in EvidenceBundleService.cs already owns that name for the Troubleshoot
/// tab's own, much broader "collect everything" ZIP bundle (msinfo32/dxdiag/systeminfo/driverquery/
/// powercfg/pnputil/event-log exports/minidumps plus this app's own findings) - both independently
/// export overlapping diagnostic data for two different tabs rather than share one implementation.
/// </summary>
public sealed class EventLogEvidenceBundleService
{
    ...
    public EventLogEvidenceBundleService()
    {
    ...
    {
        var (output, exitCode) = await ToolRunner.RunCapturedAsync(exe, args, timeoutMs, ct, timeoutOutput: "(command timed out or was cancelled)");
