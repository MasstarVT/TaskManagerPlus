...
public static class WaitChainTraversalService
{
    ...
    public static WaitChainResult Analyze(int pid, int threadId)
    {
        if (threadId <= 0)
    ...
    private static WaitChainResult Fail(string message) => new() { Success = false, IsDeadlock = false, StatusText = message };
}
