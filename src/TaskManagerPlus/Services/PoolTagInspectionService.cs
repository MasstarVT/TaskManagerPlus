...
public static class PoolTagInspectionService
{
    ...
    public static List<PoolTagDriverRow> ReadPoolTags()
    {
        int size = 1 << 20;
        for (int attempt = 0; attempt < 8; attempt++)
        {
            ...
            try
            {
                ...
                if (status != 0) return new List<PoolTagDriverRow>();

                return ParsePoolTags(buffer);
            }
            catch
            {
                return new List<PoolTagDriverRow>();
            }
            finally
            {
    ...
    private static List<PoolTagDriverRow> ParsePoolTags(IntPtr buffer)
    {
        var rows = new List<PoolTagDriverRow>();
        try
        {
